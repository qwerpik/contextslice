...
/// The built index: packages by identity, module list, vendor presence.
/// Not `Clone`: it carries the resolve-time dampener counter.
#[derive(Debug, Default)]
pub struct GoResolver {
    modules: Vec<ModuleInfo>,
    packages: BTreeMap<(String, String), Package>,
    /// Dirs that contain at least one indexed package.
    package_dirs: std::collections::BTreeSet<String>,
    /// Whether any indexed file lives under `vendor/` (enables vendor
    /// resolution; Go only honors vendor/ when it exists).
    has_vendor: bool,
    /// How often the >256-candidate dampener fired during `resolve`
    /// (threaded out of the pure binding helpers into the stats counter).
    dampened: std::sync::atomic::AtomicU64,
}
...
/// Minimal `go.mod` module-line parser: the first unindented `module X`
/// (or `module "X"`), comments skipped. `require`/`replace`/`exclude` are
/// deliberately ignored (ADR-018: replace directives would need module
/// graph resolution; repos relying on them get those imports as External).
...
/// thumb: no dot in the first element).
...
impl LanguageResolver for GoResolver {
    ...
    fn prepare(snapshot: &ResolveSnapshot) -> Self {
        ...
        let mut resolver = GoResolver {
            ...
            package_dirs: std::collections::BTreeSet::new(),
            has_vendor: false,
            dampened: std::sync::atomic::AtomicU64::new(0),
        };
        ...
        for (path, extracted) in &snapshot.files {
            ...
            let entry = resolver.packages.entry((dir, name)).or_default();
        ...
        resolver.package_dirs = resolver
            .packages
            .keys()
            .map(|(dir, _)| dir.clone())
            .collect();
...
impl GoResolver {
    ...
    fn resolve_file(
        ...
        for i in 0..refs.len() {
            let r = &refs[i];
            // A qualifier occurrence (`pkg` of `pkg.Foo`) is a package
            // reference, not a def ref: counted, never emitted.
            if r.kind == RefKind::NameRef && qualifiers.contains_key(&r.name) {
                stats.package_qualifier_refs += 1;
                continue;
            }
            let prev_qualifier = Self::preceding_qualifier(refs, i, &qualifiers);
            let before = self.dampened.load(std::sync::atomic::Ordering::Relaxed);
            let binding =
                self.bind_one_ref(r, prev_qualifier, own.as_ref(), src_is_test, &dot_packages);
            if self.dampened.load(std::sync::atomic::Ordering::Relaxed) > before {
        ...
        if src_is_test {
            for (other_id, package) in &self.packages {
                if other_id.0 == dir_of(path) && !other_id.1.starts_with("<<no-package>>") {
                    for target_file in &package.files {
    ...
    /// The binding for one reference under ADR-018's rule table.
    /// `prev_qualifier` is the package qualifier immediately preceding this
    /// ref (`pkg.Foo`), already resolved by the caller.
    fn bind_one_ref(
        &self,
        r: &cs_extract::Ref,
        prev_qualifier: Option<&QualifierTarget>,
        ...
        match r.kind {
            ...
            RefKind::TypeRef => match prev_qualifier {
                Some(QualifierTarget::Package(id)) => {
                    let hits = self.qualified_hits(id, &r.name, &type_kind);
                    let (binding, skipped) = Self::capped_or_unbound(hits);
                    if skipped {
                        self.dampened
                ...
                Some(QualifierTarget::External) => {
                    SymbolBinding::unbound(UnboundReason::ExternalScope)
                }
                None => {
            ...
            RefKind::CallRef | RefKind::FieldRef => {
                ...
                match prev_qualifier {
                    ...
                    None => match r.kind {
                        // Bare call `x.Foo()`: methods of the own package,
                        // unique only (ADR-018: `Render` is defined 18× in
                        // gin's render package), and never names from Go's
                        // universal interface surface — those collide with
                        // stdlib methods on almost any receiver.
                        RefKind::CallRef if is_universe_method(&r.name) => {
                            SymbolBinding::unbound(UnboundReason::UniverseMethod)
                        }
                        RefKind::CallRef => {
                            let methods = own
                                .as_ref()
                        ...
                        // Bare field access / method value: never bound
                        // without receiver type information.
                        _ => SymbolBinding::unbound(UnboundReason::NeedsTypeInfo),
                    },
                }
            }
        }
    ...
    /// If `refs[i]` is immediately preceded by a package-qualifier `name_ref`
    /// (`pkg.Foo`: one anonymous dot byte between the spans), return what
    /// that qualifier resolves to.
    fn preceding_qualifier<'q>(
        refs: &[cs_extract::Ref],
        i: usize,
        qualifiers: &'q BTreeMap<String, QualifierTarget>,
    ) -> Option<&'q QualifierTarget> {
        if i == 0 {
            return None;
        }
        let prev = &refs[i - 1];
    ...
    /// package, and `foo_test` is a different, test-only package). With a
    /// name conflict (invalid Go, broken repos), the lexicographically
    /// first non-test name wins, deterministically.
    fn non_test_package(&self, dir: &str) -> Option<(String, String)> {
        self.packages
            .keys()
            .filter(|(d, name)| {
                d == dir && !name.ends_with("_test") && !name.contains("<<no-package>>")
            })
            .map(|(d, name)| (d.clone(), name.clone()))
            .min()
    }
    ...
    fn resolve_import(&self, raw: &str, from_file: &str) -> Resolution {
        ...
        if raw == "." || raw == ".." || raw.starts_with("./") || raw.starts_with("../") {
            return match resolve_relative(from_dir, raw) {
                ...
                Some(dir) => self.dir_resolution(&dir, &own_module, raw),
            };
        }
        ...
        if !own_module.path.is_empty() {
            if raw == own_module.path {
                return self.dir_resolution(&own_module.dir, &own_module, raw);
            }
            let prefix = format!("{}/", own_module.path);
            if let Some(rest) = raw.strip_prefix(&prefix) {
                let candidate = join_dir(&own_module.dir, rest);
                return self.dir_resolution(&candidate, &own_module, raw);
            }
        }
        ...
        if self.has_vendor {
            ...
            if self.package_dirs.contains(&vendor_dir) {
    ...
    /// belonging to a different module — the chi `_examples` trap), or a
    /// documented miss.
    fn dir_resolution(&self, dir: &str, own_module: &ModuleInfo, raw: &str) -> Resolution {
    ...
    /// hits, with the ambiguity rule between them.
    fn bind_or_reason(
        src_is_test: bool,
        own_hits: Vec<DefLoc>,
        dot_hits: Vec<DefLoc>,
        ...
        if !own_hits.is_empty() && !dot_hits.is_empty() {
            ...
            return SymbolBinding::unbound(UnboundReason::AmbiguousDotImport);
        }
        // Go build semantics: test files are compiled only into test
        // binaries; a non-test source can never reference a test-file def.
        let mut hits: Vec<DefLoc> = own_hits
            .into_iter()
            .filter(|loc| src_is_test || !loc.file.ends_with("_test.go"))
            .collect();
        hits.extend(dot_hits);
        if hits.is_empty() || hits.len() > MAX_CANDIDATES_PER_NAME {
            SymbolBinding::unbound(UnboundReason::NoCandidate)
        } else {
            SymbolBinding::bound(hits)
        }
    }
}

/// Method names from Go's pervasive stdlib interfaces (io, fmt, context,
/// net/http handler/transport, sort). A bare `x.Close()` binds to nothing
/// even when `Close` is unique in the package: the receiver is more likely
/// an external type implementing a stdlib interface than the one local
/// definition. Measured as the dominant FP class in the gin/chi audit.
///
